<template>
  <div class="barechart">
    <ul class="label">
        <li v-for="(item, index) in devicechart" :key="index"><i :style="{ background: item.color }" />{{ item.name }}</li>
    </ul>
    <ul class="barchart">
        <li v-for="(items, index) in datapercen" :key="index" :style="{ width: items.value + '%', background: items.color }">
            <el-popover
            placement="top"
            width="230"
            trigger="hover"
...
<script>
  export default {
    ...
    data() {
        return {
            devicechart:[
                { name: '대기', value: '12', percent: '', color:'#FFC44D', nubm:'230' },
                { name: '진행', value: '47', percent: '', color:'#4EBE26', nubm:'701' },
                { name: '성공', value: '35', percent: '', color:'#2194FF', nubm:'115' },
